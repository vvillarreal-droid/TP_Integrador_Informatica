...
├── validaciones.py   ← funciones que revisan los datos antes de guardar
...
- **`class Tipo(Base)`** con **`__tablename__`**: cada clase que hereda de `Base` es una tabla. El nombre de la tabla en la base es el de `__tablename__`.
...
- **`relationship()`**: no crea una columna. Le dice a SQLAlchemy que desde un pokémon se puede llegar al objeto `Tipo` con `pokemon.tipo`. Lo usamos en la sección 5 para evitar escribir un JOIN.
...
- **`a_dict()`**: convierte el objeto en un diccionario. FastAPI sabe transformar un dict en JSON, pero no un objeto de SQLAlchemy. Es lo que devuelven todas las funciones de `crud.py`.
...
- **`datos.get("nombre")`** en vez de `datos["nombre"]`: si el campo no vino, devuelve `None` en vez de romper con `KeyError`. Así el mensaje de error lo damos nosotros.
...
- **Devolver un dict nuevo** (`limpio`) y no el original: así solo llegan a la base los campos que conocemos, aunque el usuario mande basura extra.
- **`parcial=True`**: para el `PUT` de Nivel B, donde el usuario manda solo lo que cambia. Con `parcial=False` se exige todo.

Probalo en la consola:

```python
>>> from validaciones import validar_pokemon
>>> validar_pokemon({"nombre": "Pikachu", "nivel": 25, "tipo_id": 1})
{'nombre': 'Pikachu', 'nivel': 25, 'tipo_id': 1}

>>> validar_pokemon({"nombre": "Pikachu", "nivel": 999, "tipo_id": 1})
ValueError: nivel tiene que estar entre 1 y 100

>>> validar_pokemon({"nombre": "Pikachu", "tipo_id": 1})
ValueError: nivel es obligatorio y tiene que ser un número entero

>>> validar_pokemon({"nivel": 16}, parcial=True)
{'nivel': 16}
```
...
Cada función valida (si hace falta), abre una sesión, trabaja con objetos y devuelve dicts. Quien llame a estas funciones no necesita saber nada de la base.

### Create

```python
def crear_pokemon(datos: dict) -> dict:
    limpio = validar_pokemon(datos)           # 1) validar (ValueError si está mal)
    with sesion() as s:
        pokemon = Pokemon(**limpio)           # 2) objeto Python, todavía no está en la base
        s.add(pokemon)                        # 3) lo agrego a la sesión
        s.commit()                            # 4) recién acá se escribe (y se asigna el id)
        return pokemon.a_dict()               # ya tiene el id
```

- **`Pokemon(**limpio)`**: el `**` desarma el dict en argumentos con nombre. Es lo mismo que `Pokemon(nombre="...", nivel=5, tipo_id=1)`.
...
- **`with sesion() as s:`** cierra la sesión al salir del bloque, aunque haya un error.
...
- **`select(Pokemon)`** arma la consulta (todavía no la ejecuta). Se le encadenan **`.where(...)`**, **`.order_by(...)`**, etc. Por eso se puede agregar el `where` solo si vino el filtro: es el patrón de **filtro opcional** que piden los endpoints de listado.
- **`Pokemon.tipo_id == tipo_id`** no es una comparación normal de Python: SQLAlchemy la convierte en `WHERE tipo_id = ?`. Otros que vas a usar: `Pokemon.nivel >= 10`, `Pokemon.nombre.like("%char%")`, `Pokemon.nombre.ilike(...)` (sin distinguir mayúsculas).
- **`s.scalars(consulta)`** ejecuta y devuelve los objetos uno por uno. Para traer todos en una lista: `s.scalars(consulta).all()`.
...
Un `pokemon.a_dict()` te da `tipo_id = 1`, pero no te dice que el 1 es "Fuego". Sin ORM habría que escribir un `JOIN`. Con la `relationship()` que definimos en `db.py`, alcanza con `pokemon.tipo`:
...
`p.tipo` es el objeto `Tipo` completo, con `id` y `nombre`. El `|` une dos dicts. Ojo: `p.tipo` solo funciona **adentro** del `with`, porque SQLAlchemy va a buscar el tipo a la base recién cuando lo pedís.
...
# UPDATE
crud.actualizar_pokemon(charmander["id"], {"nivel": 16})
...
|---|---|---|---|
| Nivel 999, nombre vacío, falta un campo, `"abc"` donde va un número | `validaciones.py` | `ValueError` | Antes de tocar la base |
...
| La base no existe, la tabla no existe | La base | `sqlalchemy.exc.OperationalError` | En cualquier consulta |

Nuestras validaciones no pueden saber si el tipo 999 existe porque no miran la base. La base no puede saber que el nivel máximo es 100 porque nadie se lo dijo. Cada uno controla lo suyo. Si querés, también podés chequear en `crear_pokemon` que el tipo exista con `s.get(Tipo, tipo_id)` antes del `add`, y dar un mensaje más lindo que el de la FK.
...
@app.get("/pokemones")
def listar(tipo_id: int | None = None):
    try:
...
> FastAPI trae Pydantic instalado por dentro, pero en este proyecto **no lo usamos**: las validaciones van a mano en `validaciones.py`.
...
from db import crear_tablas, sesion, Tipo, Pokemon

crear_tablas()

with sesion() as s:
    if s.scalars(select(Tipo)).first() is None:     # ¿está vacía?
        fuego = Tipo(nombre="Fuego")
        agua = Tipo(nombre="Agua")
        s.add_all([fuego, agua])
        s.commit()                                   # ahora fuego.id y agua.id existen
        s.add_all([
            Pokemon(nombre="Charmander", nivel=5, tipo_id=fuego.id),
            Pokemon(nombre="Squirtle", nivel=7, tipo_id=agua.id),
            Pokemon(nombre="Vulpix", nivel=12, tipo_id=fuego.id),
        ])
        s.commit()
...
- **Usar el objeto fuera del `with`.** `pokemon.tipo` afuera de la sesión da `DetachedInstanceError`. Convertí a dict adentro del bloque.
- **Devolver el objeto de SQLAlchemy en FastAPI.** No es JSON. Devolvé `objeto.a_dict()`.
- **`ForeignKey("Tipo.id")`.** Va el nombre de la **tabla** (`"tipos.id"`), no el de la clase.
- **Mandar el `id` al crear.** El id lo pone la base. Por eso `validar_pokemon` no lo acepta.
- **Confiar en que el tipo de la columna valida.** `Mapped[int]` no rechaza un `"25"` en texto ni un nivel 999. Lo hace `validaciones.py`.
- **`datos["nombre"]` en vez de `datos.get("nombre")`.** Si el campo no vino, rompe con `KeyError` en vez de dar un mensaje claro.
